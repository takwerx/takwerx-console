#!/usr/bin/env python3
"""TAKWERX Console v0.1.0 - Emergency Services Infrastructure Management Platform"""
...
<a href="http://{{ settings.get('server_ip', '') }}:{{ portal_port }}" target="_blank" class="cert-btn cert-btn-primary" style="text-decoration:none">👥 Open TAK Portal :{{ portal_port }}</a>
<a href="http://{{ settings.get('server_ip', '') }}:9090" target="_blank" class="cert-btn cert-btn-secondary" style="text-decoration:none">🔐 Authentik :9090</a>
<a href="https://{{ settings.get('server_ip', '') }}:8443" target="_blank" class="cert-btn cert-btn-secondary" style="text-decoration:none">🖥 TAK Server :8443</a>
</div>
</div>
...
"""
        ...
        if os.path.exists(coreconfig_path):
            ...
            if ldap_pass:
                ...
                auth_block = (
                    '    <auth default="file" x509groups="true" x509addAnonymous="false"'
...
<div style="background:var(--bg-card);border:1px solid var(--border);border-radius:12px;padding:24px;margin-bottom:24px">
<div style="display:flex;gap:12px;flex-wrap:wrap;align-items:center">
<a href="http://{{ settings.get('server_ip', '') }}:{{ ak_port }}" target="_blank" class="cert-btn cert-btn-primary" style="text-decoration:none">🔐 Authentik Admin :{{ ak_port }}</a>
<a href="http://{{ settings.get('server_ip', '') }}:3000" target="_blank" class="cert-btn cert-btn-secondary" style="text-decoration:none">👥 TAK Portal :3000</a>
<a href="https://{{ settings.get('server_ip', '') }}:8443" target="_blank" class="cert-btn cert-btn-secondary" style="text-decoration:none">🖥 TAK Server :8443</a>
</div>
<div style="font-family:'JetBrains Mono',monospace;font-size:11px;color:var(--text-dim);margin-top:12px">Admin user: <span style="color:var(--cyan)">akadmin</span> · Password auto-set during deploy (see .env file)</div>
</div>
