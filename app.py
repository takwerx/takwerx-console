...
def install_le_cert_on_8446(domain, log_fn, wait_for_cert=True):
    ...
    if wait_for_cert:
        waited = 0
        while not (os.path.exists(cert_crt) and os.path.exists(cert_key)) and waited < 60:
    ...
    try:
        ...
        if pkg_mgr == 'apt':
            ...
            cmds = [
                ...
                'curl -1sLf "https://dl.cloudsmith.io/public/caddy/stable/gpg.key" | gpg --dearmor -o /usr/share/keyrings/caddy-stable-archive-keyring.gpg 2>&1',
            ...
            for cmd in cmds:
                ...
                if 'caddy' in cmd and 'install' in cmd:
                    if r.returncode != 0:
                        plog(f"✗ Caddy install failed: {r.stderr.strip()[:200]}")
        ...
        # Step 5/5: Install LE cert on TAK Server port 8446 (ATAK enrollment)
        # Shared function handles this — works regardless of deploy order.
        if os.path.exists('/opt/tak') and os.path.exists('/opt/tak/CoreConfig.xml'):
            plog("")
            plog("━━━ Step 5/5: Installing LE Cert on TAK Server Port 8446 ━━━")
            install_le_cert_on_8446(domain, plog, wait_for_cert=True)
        else:
            plog("")
            plog("  (TAK Server not yet deployed — LE cert will be installed automatically when TAK Server is deployed)")
...
def run_takportal_deploy():
    ...
    try:
        ...
        r = subprocess.run(f'cd {portal_dir} && docker compose up -d --build 2>&1', shell=True, capture_output=True, text=True, timeout=300)
        for line in r.stdout.strip().split('\n'):
...
<a href="https://tak.{{ settings.get('fqdn', '') }}" target="_blank" style="padding:8px 14px;background:var(--bg-surface);border:1px solid var(--border);color:var(--text-secondary);border-radius:8px;font-family:'JetBrains Mono',monospace;font-size:12px;text-decoration:none">🗺️ tak</a>
<a href="https://authentik.{{ settings.get('fqdn', '') }}" target="_blank" style="padding:8px 14px;background:var(--bg-surface);border:1px solid var(--border);color:var(--text-secondary);border-radius:8px;font-family:'JetBrains Mono',monospace;font-size:12px;text-decoration:none">🔐 authentik</a>
<a href="https://takportal.{{ settings.get('fqdn', '') }}" target="_blank" style="padding:8px 14px;background:var(--bg-surface);border:1px solid var(--border);color:var(--text-secondary);border-radius:8px;font-family:'JetBrains Mono',monospace;font-size:12px;text-decoration:none">👥 takportal</a>
<a href="https://nodered.{{ settings.get('fqdn', '') }}" target="_blank" style="padding:8px 14px;background:var(--bg-surface);border:1px solid var(--border);color:var(--text-secondary);border-radius:8px;font-family:'JetBrains Mono',monospace;font-size:12px;text-decoration:none">🔴 nodered</a>
</div>
...
function pollCaddyLog(){
    ...
    var iv=setInterval(async()=>{
        try{
            ...
            if(d.entries&&d.entries.length>0){el.textContent=d.entries.join('\\n');el.scrollTop=el.scrollHeight}
...
<a href="https://{{ settings.get('fqdn', '') or settings.get('server_ip', '') }}:8446" target="_blank" class="cert-btn cert-btn-secondary" style="text-decoration:none;white-space:nowrap;font-size:12px;padding:8px 14px">🔑 WebGUI :8446 (password)</a>
</div>
</div>
...
function pollDeployLog(){
    fetch('/api/takportal/deploy/log?index='+logIndex).then(function(r){return r.json()}).then(function(d){
        ...
        if(d.running)setTimeout(pollDeployLog,1000);
            ...
            var btn=document.getElementById('deploy-btn');
            if(btn){btn.textContent='\u2713 Deployment Complete';btn.style.background='var(--green)';btn.style.opacity='1'}setTimeout(function(){window.location.href='/takportal'},3000);
        }
    });
}
...
AUTHENTIK_BOOTSTRAP_LDAP_AUTHENTIK_HOST=http://server:9000/
        ...
        if not bootstrap_token:
            ...
            for attempt in range(90):
                try:
                    ...
                    if '503' in err_str and attempt % 12 == 0:
                        plog("  Server starting up (503)...")
                    elif '403' in err_str and attempt % 12 == 0:
                        plog("  Server up, waiting for bootstrap token activation (403)...")
                    elif attempt % 12 == 0 and attempt > 0:
                        plog(f"  Still waiting... ({err_str[:60]})")
                if attempt == 0:
        ...
        try:
            ...
            if ak_token:
                ...
                if os.path.exists('/opt/tak'):
                    ...
                    if os.path.exists(tak_settings_path):
                        with open(tak_settings_path) as f:
                            tak_s = json.load(f)
                            webadmin_pass = tak_s.get('webadmin_password', 'TakserverAtak1!')
                ...
                if webadmin_pass:
                    ...
                    # Create LDAP authentication flow + stages + provider + outpost via API
                    try:
                        # Get invalidation flow
                        ...
                        auth_flow_pk = None
                        try:
                            req = urllib.request.Request(f'{ak_url}/api/v3/flows/instances/',
                                data=json.dumps({'name': 'ldap-authentication-flow', 'title': 'LDAP Authentication',
                                    'slug': 'ldap-authentication-flow', 'designation': 'authentication',
                                    'authentication': 'require_outpost'}).encode(),
                                headers=ak_headers, method='POST')
                            resp = urllib.request.urlopen(req, timeout=10)
                            auth_flow_pk = json.loads(resp.read().decode())['pk']
                            plog(f"  ✓ Created ldap-authentication-flow")
                        except urllib.error.HTTPError as e:
                            if e.code == 400:
                                req = urllib.request.Request(f'{ak_url}/api/v3/flows/instances/?slug=ldap-authentication-flow',
                                    headers=ak_headers)
                                resp = urllib.request.urlopen(req, timeout=10)
                                results = json.loads(resp.read().decode())['results']
                                auth_flow_pk = results[0]['pk'] if results else None
                                plog(f"  ✓ ldap-authentication-flow already exists")

                        if auth_flow_pk:
                            # Create password stage
                            pw_stage_pk = None
                            try:
                                req = urllib.request.Request(f'{ak_url}/api/v3/stages/password/',
                                    data=json.dumps({'name': 'ldap-authentication-password',
                                        'backends': ['authentik.core.auth.InbuiltBackend', 'authentik.core.auth.TokenBackend']}).encode(),
                                    headers=ak_headers, method='POST')
                                resp = urllib.request.urlopen(req, timeout=10)
                                pw_stage_pk = json.loads(resp.read().decode())['pk']
                            except urllib.error.HTTPError as e:
                                if e.code == 400:
                                    req = urllib.request.Request(f'{ak_url}/api/v3/stages/password/?search=ldap-authentication-password',
                                        headers=ak_headers)
                                    resp = urllib.request.urlopen(req, timeout=10)
                                    results = json.loads(resp.read().decode())['results']
                            ...
                            id_stage_pk = None
                            try:
                                req = urllib.request.Request(f'{ak_url}/api/v3/stages/identification/',
                                    data=json.dumps({'name': 'ldap-identification-stage', 'user_fields': ['username']}).encode(),
                                    headers=ak_headers, method='POST')
                                resp = urllib.request.urlopen(req, timeout=10)
                                id_stage_pk = json.loads(resp.read().decode())['pk']
                            except urllib.error.HTTPError as e:
                                if e.code == 400:
                                    req = urllib.request.Request(f'{ak_url}/api/v3/stages/identification/?search=ldap-identification-stage',
                                        headers=ak_headers)
                                    resp = urllib.request.urlopen(req, timeout=10)
                                    results = json.loads(resp.read().decode())['results']
                            ...
                            login_stage_pk = None
                            try:
                                req = urllib.request.Request(f'{ak_url}/api/v3/stages/user_login/',
                                    data=json.dumps({'name': 'ldap-authentication-login'}).encode(),
                                    headers=ak_headers, method='POST')
                                resp = urllib.request.urlopen(req, timeout=10)
                                login_stage_pk = json.loads(resp.read().decode())['pk']
                            except urllib.error.HTTPError as e:
                                if e.code == 400:
                                    req = urllib.request.Request(f'{ak_url}/api/v3/stages/user_login/?search=ldap-authentication-login',
                                        headers=ak_headers)
                                    resp = urllib.request.urlopen(req, timeout=10)
                                    results = json.loads(resp.read().decode())['results']
                                    login_stage_pk = results[0]['pk'] if results else None

                            # Bind stages to flow
                            for order, stage_pk in [(10, id_stage_pk), (15, pw_stage_pk), (20, login_stage_pk)]:
                                if stage_pk:
                                    try:
                                        req = urllib.request.Request(f'{ak_url}/api/v3/flows/bindings/',
                                            data=json.dumps({'target': auth_flow_pk, 'stage': stage_pk, 'order': order}).encode(),
                                            headers=ak_headers, method='POST')
                                        urllib.request.urlopen(req, timeout=10)
                                    except urllib.error.HTTPError:
                                        pass
                            plog(f"  ✓ Bound stages to ldap-authentication-flow")
                            ...
                            try:
                                ...
                                if e.code == 400:
                                    ...
                                    results = json.loads(resp.read().decode())['results']
                                    if results:
                                        ldap_provider_pk = results[0]['pk']
                                        # Patch to ensure cached mode and correct flows
                                        req = urllib.request.Request(f'{ak_url}/api/v3/providers/ldap/{ldap_provider_pk}/',
                                            data=json.dumps({'authentication_flow': auth_flow_pk,
                                                'bind_mode': 'cached', 'search_mode': 'cached', 'mfa_support': False}).encode(),
                                            headers=ak_headers, method='PATCH')
                                        urllib.request.urlopen(req, timeout=10)
                                        plog(f"  ✓ LDAP provider already exists, patched to cached mode")
                                else:
                                    plog(f"  ⚠ LDAP provider error: {e.code} {e.read().decode()[:100]}")
                            ...
                            if ldap_provider_pk:
                                try:
                                    ...
                                    if e.code == 400:
                                        ...
                                        plog(f"  ⚠ LDAP application error: {e.code}")
                                ...
                                try:
                                    req = urllib.request.Request(f'{ak_url}/api/v3/outposts/instances/',
                                        data=json.dumps({'name': 'LDAP', 'type': 'ldap',
                                            ...
                                            'config': {'authentik_host': 'http://server:9000/',
                                    ...
                                    plog(f"  ✓ Created LDAP outpost")
                                except urllib.error.HTTPError as e:
                                    if e.code == 400:
                                        plog(f"  ✓ LDAP outpost already exists")
                                        # Get existing outpost token_identifier
                                        ...
                                        plog(f"  ⚠ LDAP outpost error: {e.code} {e.read().decode()[:100]}")
                                ...
                                if outpost_token_id:
                                    try:
                                        ...
                                        if ldap_token_key:
                                            ...
                                            plog(f"  ⚠ Token key empty")
                                    except Exception as e:
                                        plog(f"  ⚠ Token injection error: {str(e)[:100]}")

                    except Exception as e:
                        plog(f"  ⚠ LDAP provider/outpost setup error: {str(e)[:150]}")
                else:
...
<div class="svc-card"><div class="svc-name">{{ c.name }}</div><div class="svc-status" style="color:var(--green)">● {{ c.status }}</div></div>
{% endfor %}
</div>
</div>
{% endif %}
<div class="section-title">Container Logs</div>
<div class="deploy-log" id="container-log">Loading logs...</div>
<div style="margin-top:24px;text-align:center">
...
function pollDeployLog(){
    fetch('/api/authentik/deploy/log?index='+logIndex).then(function(r){return r.json()}).then(function(d){
        ...
        if(d.running)setTimeout(pollDeployLog,1000);
            ...
            var btn=document.getElementById('deploy-btn');
            if(btn){btn.textContent='\u2713 Deployment Complete';btn.style.background='var(--green)';btn.style.opacity='1'}setTimeout(function(){window.location.href='/authentik'},3000);
        }
    });
}
...
async function loadContainerLogs(){
    ...
    if(!el)return;
    try{
        var r=await fetch('/api/authentik/logs?lines=80');
        var d=await r.json();
        el.textContent='';
        if(d.entries&&d.entries.length>0){
            d.entries.forEach(function(e){
    ...
    try:
        ...
        if fqdn:
            ...
            if caddy_active.stdout.strip() == 'active':
                ...
                install_le_cert_on_8446(fqdn, log_step, wait_for_cert=False)
