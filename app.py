...
def wait_for_apt_lock(log_fn, timeout=1800):
    """
    ...
    Waits indefinitely (up to 30 min), logging every 30s.
    """
    ...
    while waited < timeout:
        ...
        if waited % 30 == 0:
            m, s = divmod(waited, 60)
            log_fn(f"  ⏳ Still waiting... {m}m {s}s elapsed")
    log_fn(f"  ⚠ Timed out waiting for apt lock after 30m — proceeding anyway")
    return False
