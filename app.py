        ...
        try:
            ...
            if ak_token:
                ...
                if webadmin_pass:
                    ...
                    # The blueprint (tak-ldap-setup.yaml) creates the LDAP provider pointing to
                    # ldap-authentication-flow (authentication: require_outpost), and creates the
                    # LDAP outpost. We just need to wait for it to exist, then inject its token
                    # into docker-compose.yml so the LDAP container can authenticate.
                    try:
                        ...
                        # Verify the LDAP provider uses ldap-authentication-flow (require_outpost)
                        if ldap_provider_pk:
                            try:
                                ...
                                auth_flow_slug = provider_data.get('authentication_flow_obj', {}).get('slug', 'unknown')
                                plog(f"  ✓ LDAP provider authentication_flow: {auth_flow_slug}")
                            except Exception as e:
                                plog(f"  ⚠ Could not verify provider flow: {str(e)[:100]}")

                        # Inject token into docker-compose.yml and recreate LDAP container
                        if outpost_token_id:
                            try:
                                req = urllib.request.Request(
