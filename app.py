#!/usr/bin/env python3
"""TAKWERX Console v0.1.3 - Emergency Services Infrastructure Management Platform"""
...
def detect_modules():
    ...
    modules['takportal'] = {'name': 'TAK Portal', 'installed': portal_installed, 'running': portal_running,
        'description': 'User & certificate management with Authentik', 'icon': '👥', 'route': '/takportal'}
    ...
    modules['authentik'] = {'name': 'Authentik', 'installed': ak_installed, 'running': ak_running,
        'description': 'Identity provider — SSO, LDAP, user management', 'icon': '🔐', 'route': '/authentik'}
    return modules
...
def run_takportal_deploy():
    ...
    try:
        ...
        portal_settings = {
            ...
            "AUTHENTIK_PUBLIC_URL": f"http://{server_ip}:9090",
            "TAK_PORTAL_PUBLIC_URL": f"http://{server_ip}:3000",
            "TAK_URL": f"https://{server_ip}:8443/Marti",
        ...
        plog(f"  Authentik URL: http://{server_ip}:9090")
        plog(f"  TAK Server URL: https://{server_ip}:8443/Marti")
        if ak_token:
...
<a href="http://{{ settings.get('server_ip', '') }}:{{ portal_port }}" target="_blank" class="cert-btn cert-btn-primary" style="text-decoration:none;white-space:nowrap;font-size:12px;padding:8px 14px">👥 TAK Portal :{{ portal_port }}</a>
<a href="http://{{ settings.get('server_ip', '') }}:9090" target="_blank" class="cert-btn cert-btn-secondary" style="text-decoration:none;white-space:nowrap;font-size:12px;padding:8px 14px">🔐 Authentik :9090</a>
<a href="https://{{ settings.get('server_ip', '') }}:8443" target="_blank" class="cert-btn cert-btn-secondary" style="text-decoration:none;white-space:nowrap;font-size:12px;padding:8px 14px">🔐 WebGUI :8443 (cert)</a>
<a href="https://{{ settings.get('server_ip', '') }}:8446" target="_blank" class="cert-btn cert-btn-secondary" style="text-decoration:none;white-space:nowrap;font-size:12px;padding:8px 14px">🔑 WebGUI :8446 (password)</a>
</div>
</div>
...
AUTHENTIK_BOOTSTRAP_LDAPSERVICE_USERNAME=ldapservice
...
"""
        ...
        plog("  LDAP service account: ldapservice")
        ...
        # Step 8: Wait for API, retrieve LDAP outpost token
        plog("")
        plog("\u2501\u2501\u2501 Step 8/10: Configuring LDAP Outpost Token \u2501\u2501\u2501")
        ...
        if not bootstrap_token:
            plog("\u26a0 No bootstrap token, LDAP token needs manual config")
        else:
            import urllib.request
            import json as json_mod
            api_ready = False
            ...
            for attempt in range(90):
                try:
                    req = urllib.request.Request(
                        'http://127.0.0.1:9090/api/v3/outposts/instances/',
                        headers={'Authorization': f'Bearer {bootstrap_token}'}
                    )
                    resp = urllib.request.urlopen(req, timeout=5)
                    data = json_mod.loads(resp.read().decode())
                    results = data.get('results', [])
                    status = f"{len(results)} outpost(s): {', '.join(r.get('name','?')+'/'+r.get('type','?') for r in results)}"
                    if status != last_status:
                        plog(f"  API responding - {status}")
                        last_status = status
                    for outpost in results:
                        if outpost.get('name') == 'LDAP' and outpost.get('type') == 'ldap':
                            api_ready = True
                            break
                    if api_ready:
                        plog("\u2713 LDAP outpost found")
                        break
            ...
            if api_ready:
                # Find the LDAP outpost specifically (not embedded proxy)
                ldap_outpost = None
                for outpost in data.get('results', []):
                    if outpost.get('name') == 'LDAP' and outpost.get('type') == 'ldap':
                        ldap_outpost = outpost
                        break
                if not ldap_outpost:
                    # Fallback: any non-embedded ldap outpost
                    for outpost in data.get('results', []):
                        if outpost.get('managed') != 'goauthentik.io/outposts/embedded' and outpost.get('type') == 'ldap':
                            ldap_outpost = outpost
                            break
                if not ldap_outpost:
                    plog("\u26a0 LDAP outpost not found - blueprint may still be processing")
                    plog("  Try redeploying or check Admin > Outposts")
                else:
                    token_id = ldap_outpost.get('token_identifier', '')
                    plog(f"  Found LDAP outpost: {ldap_outpost.get('name', 'LDAP')}")
                    try:
                        req2 = urllib.request.Request(
                            f'http://127.0.0.1:9090/api/v3/core/tokens/{token_id}/view_key/',
                            headers={'Authorization': f'Bearer {bootstrap_token}'}
                        )
                        resp2 = urllib.request.urlopen(req2, timeout=5)
                        key_data = json_mod.loads(resp2.read().decode())
                        ldap_token_key = key_data.get('key', '')
                        if ldap_token_key:
                            with open(compose_path, 'r') as f:
                                compose_text = f.read()
                            ...
                            plog("\u2713 LDAP outpost token configured")
                        else:
                            plog("\u26a0 Token key empty - check Admin > Directory > Tokens")
                    except Exception as e:
                        plog(f"\u26a0 Token error: {str(e)}")
            else:
                plog("\u26a0 API timeout - LDAP token needs manual config")

        # Step 9: Start LDAP outpost
        plog("")
        plog("\u2501\u2501\u2501 Step 9/10: Starting LDAP Outpost \u2501\u2501\u2501")
        ...
        plog("")
        plog("\u2501\u2501\u2501 Step 10/10: Connecting TAK Server to LDAP \u2501\u2501\u2501")
        ...
        if os.path.exists(coreconfig_path):
            ...
            if ldap_pass:
                ...
                auth_block = (
                    ...
                    f' serviceAccountDN="cn=ldapservice,ou=users,dc=takldap"'
        ...
        plog(f"  - Service account: ldapservice")
        if ldap_svc_pass:
...
<div style="background:var(--bg-card);border:1px solid var(--border);border-radius:12px;padding:24px;margin-bottom:24px">
<div style="display:flex;gap:10px;flex-wrap:nowrap;align-items:center">
<a href="http://{{ settings.get('server_ip', '') }}:{{ ak_port }}" target="_blank" class="cert-btn cert-btn-primary" style="text-decoration:none;white-space:nowrap;font-size:12px;padding:8px 14px">🔐 Authentik :{{ ak_port }}</a>
<a href="http://{{ settings.get('server_ip', '') }}:3000" target="_blank" class="cert-btn cert-btn-secondary" style="text-decoration:none;white-space:nowrap;font-size:12px;padding:8px 14px">👥 TAK Portal :3000</a>
<a href="https://{{ settings.get('server_ip', '') }}:8443" target="_blank" class="cert-btn cert-btn-secondary" style="text-decoration:none;white-space:nowrap;font-size:12px;padding:8px 14px">🔐 WebGUI :8443 (cert)</a>
<a href="https://{{ settings.get('server_ip', '') }}:8446" target="_blank" class="cert-btn cert-btn-secondary" style="text-decoration:none;white-space:nowrap;font-size:12px;padding:8px 14px">🔑 WebGUI :8446 (password)</a>
</div>
...
<div><span style="color:var(--text-dim)">Service Account:</span> <span style="color:var(--cyan)">ldapservice</span></div>
...
<div style="background:var(--bg-card);border:1px solid var(--border);border-radius:12px;padding:24px;margin-bottom:24px">
<div style="display:flex;gap:10px;flex-wrap:nowrap;align-items:center">
<a href="https://{{ settings.get('server_ip', '') }}:8443" target="_blank" class="cert-btn cert-btn-primary" style="text-decoration:none;white-space:nowrap;font-size:12px;padding:8px 14px">🔐 WebGUI :8443 (cert)</a>
<a href="https://{{ settings.get('server_ip', '') }}:8446" target="_blank" class="cert-btn cert-btn-secondary" style="text-decoration:none;white-space:nowrap;font-size:12px;padding:8px 14px">🔑 WebGUI :8446 (password)</a>
<a href="http://{{ settings.get('server_ip', '') }}:3000" target="_blank" class="cert-btn cert-btn-secondary" style="text-decoration:none;white-space:nowrap;font-size:12px;padding:8px 14px">👥 TAK Portal :3000</a>
<a href="http://{{ settings.get('server_ip', '') }}:9090" target="_blank" class="cert-btn cert-btn-secondary" style="text-decoration:none;white-space:nowrap;font-size:12px;padding:8px 14px">🔐 Authentik :9090</a>
</div>
