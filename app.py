...
def wait_for_apt_lock(log_fn, log_list, timeout=1800):
    """
    Wait for unattended-upgrades / apt locks to release before installing packages.
    log_fn: timestamped logging function for normal messages
    log_list: the raw log list to append ticker lines directly (no timestamp prefix)
    """
    ...
    log_fn("  ⚠ Timed out waiting for apt lock after 30m — proceeding anyway")
    return False
                            ...
                            if ldap_provider_pk:
                                ...
                                try:
                                    ...
                                    if e.code == 400:
                                        ...
                                        # Get existing outpost and patch providers + token
                                        ...
                                        if ldap_outpost:
                                            ...
                                            urllib.request.urlopen(req, timeout=10)
                                            plog(f"  ✓ LDAP outpost provider linked")
                                    else:
                                ...
                                if outpost_token_id:
                                    try:
                                        ...
                                        if ldap_token_key:
                                            ...
                                            plog(f"  ⚠ Token key empty — response was: {response_body[:200]}")
                                    except urllib.error.HTTPError as e:
...
<div class="svc-card" onclick="filterLogs('{{ c.name }}')" style="cursor:pointer" id="svc-{{ c.name }}"><div class="svc-name">{{ c.name }}</div><div class="svc-status" style="color:var(--green)">● {{ c.status }}</div></div>
{% endfor %}
