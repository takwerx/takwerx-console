            ...
            if ak_token:
                ...
                for attempt in range(60):  # up to 5 minutes
                    try:
                        ...
                        token_ok = True
                        plog(f"  ✓ Bootstrap token active (attempt {attempt+1})")
                        break
                    except urllib.error.HTTPError as e:
                        if e.code == 403:
                            if attempt % 6 == 0:
                            ...
                            plog(f"  ⚠ Token check unexpected error: {e.code}")
                            break
                    except Exception as e:
                        plog(f"  ⚠ Token check error: {str(e)[:80]}")
                        time.sleep(5)

                if not token_ok:
                    plog("  ⚠ Bootstrap token not working after 5 minutes — skipping admin setup")
                    raise Exception("Bootstrap token not working")
                ...
                if webadmin_pass:
                    ...
                    try:
                        ...
                        auth_flow_pk = next((f['pk'] for f in auth_flows if f['slug'] == 'default-authentication-flow'), auth_flows[0]['pk'] if auth_flows else None)
